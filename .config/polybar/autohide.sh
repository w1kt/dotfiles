...
function main
{
  if [[ "$1" == "setup" ]]; then
    move_windows -24
    xdotool search --name polybar-top_eDP-1 behave %@ mouse-enter exec --sync "$XDG_CONFIG_HOME"/polybar/autohide 
show &
    xdotool search --name polybar-top_eDP-1 behave %@ mouse-leave exec --sync "$XDG_CONFIG_HOME"/polybar/autohide 
hide &
